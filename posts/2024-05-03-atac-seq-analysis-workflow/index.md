...
ATAC-Seq（Assay for Transposase-Accessible Chromatin with high-throughput Sequencing）技术依赖于使用高活性转座酶 Tn5 的文库构建和高通量测序。ATAC-Seq 方法将 NGS 接头连接到转座酶上，该转座酶可以使染色质断裂并同时将这些接头整合到开放的染色质区域中。构建的文库通过 NGS 测序，并使用生物信息学分析具有可及或可访问染色质的基因组区域。
...
peakAnnoList <- lapply(peaks, annotatePeak, tssRegion = c(-3000, 3000), TxDb = txdb, annoDb = "org.Oryza.sativa.eg.db")
plotAnnoBar(peakAnnoList)
```

![](assets/20240503154417.webp)
...
然后使用 TBtools 的 circos 绘图工具，只导入染色体长度文件，然后修改 macs2 输出文件中的 narrowPeak 文件，只保留染色体、开始位置、终止位置和 q-value/p-value 值四列即可，如：
...
![](assets/20240503155145.webp)

然后改一下颜色、字体什么的进行一下简单的美化：

![](assets/20240503155277.webp)
