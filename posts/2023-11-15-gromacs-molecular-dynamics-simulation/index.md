...
以β-木糖醇酶的蛋白文件为基础，前提是要去除小分子，否则 gromacs 会报错提示有非标准残基。
...
```bash
conda create -n gmx python=3.9 -y
conda activate gmx
conda install -c bioconda gromacs -y
```

安装后可能出现 WARNING，若需要可安装 pocl：
```bash
...
选择合适力场（示例中选择 OPLS-AA/L）。

记录体系电荷（示例：Total charge 9.000 e）。
...
能量/温度/压力等结果用 gmx energy 导出并用 DuIvyTools 的 dit 查看（按提示选择相应编号）。
